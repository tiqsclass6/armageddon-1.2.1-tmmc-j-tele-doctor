...
```text
Armageddon 1.2.1/
├── README.md               # project front page (architecture + how to start)
├── terraform/              # init / fmt / validate / plan / apply
├── scripts/
│   ├── 1-user-data.sh          # Apache + Promtail
│   ├── 2-grafana.sh            # Loki + Grafana
│   ├── 3-run-lab.sh            # deploy + live E2E (phases 01/15–15/15)
│   ├── 4-teardown-lab.sh       # empty syslog buckets + terraform destroy
│   └── render-architecture.py  # redraw documentation/Armageddon.png
└── documentation/          # this runbook, CIDR sheet, architecture PNG, spec
```
...
2. `02/15` `terraform init`
3. `03/15` `terraform fmt`
4. `04/15` `terraform validate`
5. `05/15` `terraform plan`
6. `06/15` `terraform apply`
...
5. Optional SSM: `aws ssm start-session --region ap-northeast-1 --target i-<siem>` then `curl http://127.0.0.1:3100/ready`.
6. Console: Tokyo `1d` has no public subnet; SIEM vs DB CIDRs differ.

Default Grafana UI (from a Tokyo VPC host only): `admin` / `admin`.
...
| `scripts/render-architecture.py` | Redraw `documentation/Armageddon.png` |
